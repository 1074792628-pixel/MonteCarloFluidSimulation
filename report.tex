...
下面考察当前仿真的局限性。在 $N_{\text{walk}} \le 512$ 时，WoS 求解的估值噪声 $\sim O(1/\sqrt{N_{\text{walk}}})$ 通过中心差分梯度算子被放大 $\sim 1/(2h)$ 倍，导致速度修正的误差与待修正量同量级，使得投影步骤失效。若要达到稳定仿真，需要将 $N_{\text{walk}}$ 提升至 $10^4$--$10^5$ 量级，这需要 GPU 加速。
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item \textbf{噪声影响}：Monte Carlo 估计的统计噪声通过数值差分操作被放大约 $1/(2h)$ 倍，是限制精度的核心瓶颈。在 $N_{\text{walk}} = 256$ 时，动能呈现非物理增长（20.0 $\to$ 185.8）。稳定仿真需要 $N_{\text{walk}} \geq 10^4$。
    \item \textbf{计算效率}：$10\times10$ 网格、256 次随机行走的单步计算时间约为 0.4 秒（含两个求解器）。
    \item \textbf{GPU 编译}：NVIDIA WoSX 框架 \cite{sawhney2023wosx} 已成功编译（含 Python 绑定和 GPU 支持）。VelMCFluids \cite{sugimoto2024code} 因 CUDA 13.3 与旧版 OWL 库的 API 不兼容暂未编译成功，需降级至 VS 2022 或在 Linux 上编译。
\end{enumerate}
