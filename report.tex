...
\begin{abstract}
本实验实现了两种基于 Monte Carlo 的不可压缩流体仿真方法的 \textbf{GPU 版本}，并进行了定量对比与误差来源分析：
...
本实验用 CuPy 将 2022 涡量法的核心 Biot-Savart 积分 GPU 化（完美并行负载，加速约 120 倍），并编译运行了 2024 论文的官方 OptiX GPU 实现（VelMCFluids）。
在粘性 Taylor-Green 涡（$\nu=0.05$，有解析解）基准上进行了 GPU vs GPU 定量对比。
结果表明：在 $64\times64$ 网格上，2022 GPU 的涡量 RMSE（0.51）低于 2024 GPU（1.25--0.95），且耗时低约 37 倍（0.13 s vs 4.84 s）。
误差来源分析显示：2022 的误差受限于半拉格朗日平流插值耗散（存在平台约 0.51），而 2024 的误差受限于 WoB 投影的 MC 噪声（随采样数持续下降但未收敛）。
\end{abstract}
...
本实验对比实现了这两种算法的 GPU 版本：
\begin{itemize}
    \item \textbf{2022 涡量法 GPU 版}：用 CuPy 将 Biot-Savart 积分向量化（\texttt{gpu\_fluid\_2022.py}）；
    \item \textbf{2024 速度法 GPU 版}：编译 2024 论文官方 OptiX 实现（\texttt{VelMCFluids/}）。
\end{itemize}
...
\subsection{2022 涡量法实现（GPU）}

2022 涡量法的 GPU 版（\texttt{gpu\_fluid\_2022.py}）用 \textbf{CuPy} 将核心 Biot-Savart 积分向量化到 GPU。Biot-Savart 速度重建 $\boldsymbol{u}(\boldsymbol{x}) = \frac{1}{N}\sum_k \boldsymbol{B}(\boldsymbol{x},\boldsymbol{y}_k)\omega(\boldsymbol{y}_k)$ 是\textbf{完美并行}负载——每个查询点独立，GPU 化后加速约 120 倍。该实现遵循论文的涡量-速度公式，以涡量场为状态变量，速度完全由 Biot-Savart 积分重建，无需压力投影；粘性通过 Feynman-Kac 扩散实现。

\subsection{2024 速度法实现（GPU）}

2024 速度法的 GPU 版（\texttt{VelMCFluids/}）是论文作者的官方 OptiX 参考实现，包含完整的 WoB 边界积分投影（奇异体积分、伪边界项、速度源项、边界路径积分），通过 CUDA/OptiX 光线追踪加速。本实验在 Windows 上完成了编译（需 CUDA 13.3 + OptiX 9.1 + 新版驱动 610.88），并跑通了论文场景。

\section{实验结果与分析}

\subsection{实验设置}

为进行真正对等的\textbf{GPU vs GPU}对比，本实验：
\begin{itemize}
    \item 用 \textbf{CuPy} 将 2022 涡量法 GPU 化（核心 Biot-Savart 积分是完美并行负载，向量化到 GPU）；
    \item 编译运行 2024 论文官方 \textbf{OptiX GPU} 实现（VelMCFluids，需 CUDA 13.3 + OptiX 9.1 + 驱动 610.88）。
\end{itemize}

测试采用\textbf{粘性 Taylor-Green 涡}（$\nu=0.05$，有解析解）作为基准，$64\times64$ 网格，$\Delta t=0.05$，对比涡量误差（RMSE vs 解析解）与耗时。

\subsection{GPU 版定量对比结果}
...
\textbf{2022 涡量法的优势与代价。} 涡量法的核心优势是\textbf{天然不可压缩性}：速度由 Biot-Savart 积分重建，自动满足 $\nabla\cdot\boldsymbol{u}=0$，避免了投影步骤及其噪声。在本实验的 GPU 对比中，Biot-Savart 积分是完美并行负载，CuPy GPU 化后既快（40 步 0.13 s）又准（RMSE 0.51）。其代价是三维中需处理涡拉伸项，且在非单连通区域面临调和场问题\cite{sugimoto2024}。

\textbf{2024 速度法的优势与代价。} 速度法直接操作速度场，采用算子分裂框架，可自然融入浮力、PIC/FLIP、平流-反射等传统速度法技术，其核心贡献是 WoB 边界积分投影，适合复杂几何边界。其代价是投影步骤在有限 MC 采样下引入噪声——本实验中即便增至 1024 采样，误差（0.95）仍高于 2022 涡量法的插值平台（0.51），且单步成本更高（40 步 4.84 s）。

\textbf{选择建议。} 两种方法各有适用场景：涡量法在平滑流场上精度更高、GPU 化后效率更高，适合强调物理守恒的研究性仿真（涡旋动力学、环量分析）；速度法适合处理复杂几何边界的动画级应用，可方便地扩展浮力、FLIP 等效果。

\section{结论与展望}

本实验实现了 2022 年 Rioux-Lavoie et al. 涡量法和 2024 年 Sugimoto et al. 速度法的 GPU 版本，并在粘性 Taylor-Green 基准上进行了定量对比与误差来源分析。主要结论如下：

\begin{enumerate}[label=(\arabic*)]
    \item \textbf{两种 GPU 算法均已实现}：2022 涡量法（CuPy GPU 化 Biot-Savart）和 2024 速度法（官方 OptiX 实现）均稳定运行。
    \item \textbf{定量精度}：在 $64\times64$、256 采样下，2022 GPU 的涡量 RMSE（0.514）低于 2024 GPU（1.247）；2024 增至 1024 采样（0.947）仍高于 2022。
    \item \textbf{计算效率}：相同分辨率与采样下，2022 GPU 比 2024 GPU 快约 \textbf{37 倍}（0.13 s vs 4.84 s，40 步）。
