...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{report_results/2022_vs_2024_algorithm.png}
    \caption{2022 涡量法与 2024 速度法的动能演化对比（$16\times16$ 网格，无粘）。}
    \label{fig:ke_comparison}
\end{figure}

\begin{table}[H]
    \centering
    \caption{2022 涡量法与 2024 速度法对比结果}
    \label{tab:2022_2024}
    \rowcolors{2}{white}{lightgray}
    \begin{tabular}{c|ccc|c}
        \toprule
        \textbf{方法} & \textbf{初KE} & \textbf{终KE} & \textbf{最大涡量终值} & \textbf{15步耗时} \\
        \midrule
        2022 涡量法 (nmc=256) & 48.9 & 35.1 & 5.35 & 7.5 s \\
        2022 涡量法 (nmc=512) & 50.3 & 30.6 & 5.48 & 11.2 s \\
        2024 速度法 (256p+64r) & 44.9 & 24.2 & 3.44 & 0.4 s \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{物理量守恒}：2022 涡量法对最大涡量的保持更好（$\omega_{\max}$ 从 6.04 仅降至 5.35，保留 89\%），而 2024 速度法降至 3.44（保留 60\%）。这印证了涡量法直接演化涡量、由 Biot-Savart 重建速度的物理保真优势。

    \item \textbf{计算效率}：2022 涡量法每步需要为每个网格点执行全域 Biot-Savart 积分（$N$ 个采样），耗时 7.5--11.2 s；2024 速度法借助网格缓存的投影步骤，仅需 0.4 s，快约 \textbf{18--28 倍}。这是涡量法的主要短板。

    \item \textbf{稳定性}：两种方法在各自框架下均稳定运行。涡量法因速度由积分重建而无投影噪声问题；速度法依赖投影步骤，但 WoB 投影的噪声是步间独立的，不形成正反馈发散。

    \item \textbf{数值耗散}：两种方法在无粘条件下动能均衰减（涡量法 -28\%，速度法 -46\%），这是半拉格朗日平流插值和网格缓存的固有数值耗散。速度法的耗散更大，部分源于投影步骤的平滑处理。
\end{itemize}

\subsection{讨论}

\textbf{2022 涡量法的优势与代价。} 涡量法的核心优势是\textbf{物理守恒性}：涡量直接作为状态变量演化，速度由积分重建，避免了压力投影引入的数值误差，也天然满足不可压缩性。其代价是 Biot-Savart 积分的计算开销——每次速度查询都需要遍历全域采样，在细网格上成本急剧上升。此外，涡量法在三维中需要处理涡拉伸项，且涡量-速度转换在非单连通区域面临调和场问题\cite{sugimoto2024}。
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item \textbf{物理守恒}：涡量法对涡量/环量的保持优于速度法（$\omega_{\max}$ 保留 89\% vs 60\%），因其直接演化涡量且速度由积分重建。
    \item \textbf{计算效率}：速度法快约 18--28 倍（0.4 s vs 7.5--11.2 s 每 15 步），网格缓存使投影步骤成本远低于全域 Biot-Savart 积分。
